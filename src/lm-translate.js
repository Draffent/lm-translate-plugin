// LM Studio Translate v2.1 — 翻译 + 数据采集 + LLM 桥接 (tsApi 通道)
(function(){
'use strict';
...
async function checkProxyHealth() {
  try {
    var data = await proxyGet(PROXY_URL + '/health');
...
async function tsFetch(url) {
  ...
  try {
    ...
    if (q) {
      var data = await proxyGet(PROXY_URL + '/translate?q=' + encodeURIComponent(q) +
...
async function baiduBatch(texts, from, to) {
  ...
  try {
    ...
    var data = await proxyPost(PROXY_URL + '/batch', { texts: texts, from: from, to: to });
    if (data && data.ok && data.translations) {
